...
# TODO: run driver_scripts/drv_compute_GC_presets.sh first!
gcparagon_matrices_parent_dir = MY_ABSOLUTE_SOURCE_DIRECTORY_PATH / ('validation/01_transformed_Griffin_bias_matrices/'
                                                                     'GCparagon_matrices_per_preset')

output_path = MY_ABSOLUTE_SOURCE_DIRECTORY_PATH / \
              'validation/01_transformed_Griffin_bias_matrices'
# a subdirectory will be created in there "difference_plots_GCparagon-Griffin" for each preset
...
# target (simulated) matrix: in silico construct from GC weights and fragment frequencies!
...
for preset in range(1, 3, 1):  # no preset 3 validation yet
    ...
    for w_mat in weights_matrices:
        ...
        assert sample_name not in gcparagon_matrices_collection[preset_label]['weights']
        # ^--- only valid in first iteration!
...
if __name__ == '__main__':
    ...
    for sample_bias_matrix in sample_bias_matrices:
        ...
        sample_target_attributes_matrix, flen_range_trg = load_txt_to_matrix_with_meta(
            ...
            to_dtype=np.uint64)
        # insanity check
        assert flen_range_mask == flen_range_obs == flen_range_trg == flen_range_weights
        ...
        for preset_label in gcparagon_matrices_collection.keys():
            ...
            # OBSERVATIONS: (scale to 100k fragments dataset size -> prevent overflow errors)
            theoretical_possibilities_gcp = ((flen_range_obs_gcp[1] - flen_range_obs_gcp[0]) *  # fragment lengths
                                             ...
                                             ((flen_range_obs_gcp[1] - flen_range_obs_gcp[0]) ** 2) / 2)  # not possible
            ...
            for data_id in frq_data_diff.keys():
                ...
                if data_id == 'W_gc':
                    ...
                    plot_statistic_matrices(frq_data={'W_gc': pd.DataFrame(weights_differences_focused_2_limited)},
                                            ...
                                            fig_width=1800, fig_height=2000, fig_fontsize=50, parent_logger=cmd_logger)
            # 2) plot differences in fragment length distributions
            ...
            plot_fragment_length_dists(matrix_data_frame=fragment_occurrences_per_sample, fig_fontsize=32,
                                       ...
                                       # DON'T NORMALIZE! Should not sum up to 1!!!
