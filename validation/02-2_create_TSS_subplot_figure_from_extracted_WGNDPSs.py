...
show_figure = False  # set to True for interactive plot in browser
...
CONTENT_ROOT_DIR = MY_ABSOLUTE_SOURCE_DIRECTORY_PATH
...
TSS_COLORS = {'HK': 'rgba(190, 50, 0, 0.85)',
              'PAU': 'rgba(150, 150, 150, 0.85)'}
SAMPLE_COLORS = {'B01': 'rgba(51, 98, 255, 0.85)',
                 'C01': 'rgba(255, 115, 12, 0.85)',
                 'H01': 'rgba(0, 178, 18, 0.85)',
                 'P01': 'rgba(255, 0, 0, 0.85)'}

# TSSs DoC definitions:
tss_coverage_csv_paths = {'GCparagon': {'preset1': CONTENT_ROOT_DIR / 'validation/02_loci_overlay_central_coverages/'
                                                                      'GCparagon/preset1',
                                        'preset2': CONTENT_ROOT_DIR / 'validation/02_loci_overlay_central_coverages/'
                                                                      'GCparagon/preset2'},
                          'Griffin': CONTENT_ROOT_DIR / 'validation/02_loci_overlay_central_coverages/Griffin',
                          'Original': CONTENT_ROOT_DIR / 'validation/02_loci_overlay_central_coverages/notCorrected'}

output_file_str = str(CONTENT_ROOT_DIR /
                      'validation/02_loci_overlay_central_coverages/coverage_overlay_plot_output/'
                      'DoC_bias_correction_effect_TSSs_{}.png')  # MUST be formatted with preset
REF_GC_DUMP = CONTENT_ROOT_DIR / 'accessory_files/TSSs/TSS_ref_gc_content.npydct'
# created in the first iteration;  MUST be deleted if the reference regions in tss_coverage_csv_paths change!


def load_tss_regions_from_bed_file(region_list_dict: Dict[str, Union[str, Path]], library_file=None, stranded=False) \
...
def get_multi_signals_plot(plot_data_signals: Dict[str, Dict[str, np.array]], preset: str,
    ...
    for signal_id in signal_id_ordered:  # HK, PAU
        for status_id in plot_data_signals[signal_id]:  # original, corrected
            if status_id == 'corrected':
                signal = plot_data_signals[signal_id][status_id][preset]  # np.array
            else:
...
def plot_ref_gc_content(data_to_plot: Dict[str, Dict[str, np.array]], transparencies: Dict[str, float],
                        signal_colors: Dict[str, str], fig_fontsize=FIGURE_FONT_SIZE,
...
def main() -> int:
    ...
    # ------------------------------------------------------------------------------------------------------------------
    if REF_GC_DUMP.is_file():  # fast - should be the deafult (reference genome GC content has already been computed)
        ...
        # read target genes
        hg38_ref_regions, stranded_data = load_tss_regions_from_bed_file(
            region_list_dict=target_regions_per_class, library_file=target_region_library_bed_path, stranded=True)
    ...
    # ------------------------------------------------------------------------------------------------------------------
    # OUTDATED stem_path = Path(tss_coverage_csv_paths['stem'])
    ...
    cov_data = {}
    for corr_algo, npz_parent_dir_dict in tss_coverage_csv_paths.items():   # iter over correction state
        if corr_algo == 'GCparagon':
            presets = tss_coverage_csv_paths[corr_algo].keys()
            for preset in presets:
                npz_candidates = tuple(npz_parent_dir_dict[preset].glob('*.npz'))
                if len(npz_candidates) == 0:
                    print(f"WARNING: no csv file found under path '{npz_parent_dir_dict[preset]}'. "
                          f"Ignoring this directory ..")
                    continue
                for npz_candidate in npz_candidates:  # iter over samples
                    region_class = npz_candidate.stem.split('_')[1]
                    sample_name = npz_candidate.stem.split('_')[0]
                    if corr_algo == 'Original':
                        signal_type = 'original'
                    elif corr_algo == 'GCparagon':
                        signal_type = 'corrected'
                    else:
                        assert corr_algo == 'Griffin'
                        signal_type = 'corrected (Griffin)'
                    c61_filtered_average_covs = np.load(npz_candidate)['trimmed_mean_of_normalized_signals']
                    ...
                    if cov_data.get(sample_name) is None:
                        cov_data.update({sample_name: {region_class: {signal_type: {}.fromkeys(presets)}}})
                    elif cov_data[sample_name].get(region_class) is None:
                        cov_data[sample_name].update({region_class: {signal_type: {}.fromkeys(presets)}})
                    elif cov_data[sample_name][region_class].get(signal_type) is None:
                        cov_data[sample_name][region_class].update({signal_type: {}.fromkeys(presets)})
                    elif cov_data[sample_name][region_class][signal_type].get(preset) is not None:
                        raise AssertionError(f"INSANITY - loaded coverage data already exists for algorithm "
                                             f"'{corr_algo}', sample '{sample_name}', region group '{region_class}' "
                                             f"and {preset} - should not be the case!")
                    cov_data[sample_name][region_class][signal_type][preset] = c61_filtered_average_covs
        else:  # Griffin corrected data (no preset distinction needed here)
            npz_parent_dir = npz_parent_dir_dict
            npz_candidates = tuple(npz_parent_dir.glob('*.npz'))
            if len(npz_candidates) == 0:
                print(f"WARNING: no csv file found under path '{npz_parent_dir}'. Ignoring this directory ..")
                continue
            for npz_candidate in npz_candidates:  # iter over samples
                region_class = npz_candidate.stem.split('_')[1]
                sample_name = npz_candidate.stem.split('_')[0]
                ...
                if cov_data.get(sample_name) is None:
                    ...
                    raise AssertionError(f"INSANITY - loaded coverage data already exists for algorithm '{corr_algo}', "
                                         f"sample '{sample_name}' and region group '{region_class}' "
                                         f"({signal_type}) - should not be the case!")
                cov_data[sample_name][region_class][signal_type] = c61_filtered_average_covs

    # ACTUALLY CREATE PLOTS:
    # --------------------------------------------------------------------------------------------------------------
    for preset_str in tss_coverage_csv_paths['GCparagon'].keys():
        # create single column, 7 rows multiplot:
        ...
        ref_gc_content_traces = plot_ref_gc_content(data_to_plot=data_to_plot, transparencies=transparent_signals,
                                                    ...
                                                    fig_fontsize=FIGURE_FONT_SIZE, fig_width=1000, fig_height=1000)
        if ref_gc_content_traces == 1:
            raise ValueError("something went wrong during plotting of reference genome GC content")
        ...
        b01_hkpau_traces = get_multi_signals_plot(plot_data_signals=cov_data['B01'], annotation='B01',
                                                  ...
                                                  signal_dash={'corrected': 'solid',
                                                               'original': 'dot',
                                                               'corrected (Griffin)': 'dash'},
        ...
        for doc_trace in b01_hkpau_traces:
            if 'HK' in doc_trace.name:
                if 'corrected' in doc_trace.name:
                    if 'Griffin' in doc_trace.name:
                        doc_trace.name = 'B01 HK corrected (Griffin)'
                ...
                if 'corrected' in doc_trace.name:
                    if 'Griffin' in doc_trace.name:
                        doc_trace.name = 'B01 PAU corrected (Griffin)'
                    else:
        ...
        h01_hkpau_traces = get_multi_signals_plot(plot_data_signals=cov_data['H01'], annotation='H01',
                                                  signal_colors=SAMPLE_COLORS,
                                                  x_axis_data_label='relative position / bp',
                                                  y_axis_data_label='relative coverage / 1',
                                                  signal_dash={'corrected': 'solid',
                                                               'original': 'dot',
                                                               'corrected (Griffin)': 'dash'},
                                                  figure_width=1000, figure_height=380, preset=preset_str)
        ...
        for doc_trace in h01_hkpau_traces:
            if 'HK' in doc_trace.name:
                if 'corrected' in doc_trace.name:
                    if 'Griffin' in doc_trace.name:
                        doc_trace.name = 'H01 HK corrected (Griffin)'
                    else:
                ...
                if 'corrected' in doc_trace.name:
                    if 'Griffin' in doc_trace.name:
                        doc_trace.name = 'H01 PAU corrected (Griffin)'
                    else:
        ...
        for doc_trace in p01_hkpau_traces:
            if 'HK' in doc_trace.name:
                if 'corrected' in doc_trace.name:
                    if 'Griffin' in doc_trace.name:
                        doc_trace.name = 'P01 HK corrected (Griffin)'
                    else:
                ...
                if 'corrected' in doc_trace.name:
                    if 'Griffin' in doc_trace.name:
                        doc_trace.name = 'P01 PAU corrected (Griffin)'
                    else:
        ...
        for dat_idx in plotting_order:
            ...
            if 'filtered' in trace_name:
                ...
                fig.data[dat_idx].showlegend = False
            # set Griffin line specs
            if 'Griffin' in trace_name:
                fig.data[dat_idx].line.width = 3.
                fig.data[dat_idx].line.dash = 'solid'
                fig.data[dat_idx].line.color = 'rgba(30, 30, 30, 0.75)'
                fig.data[dat_idx].name = 'corrected (Griffin)'
                if griffin_shown:
                    fig.data[dat_idx].showlegend = False  # only show once in legend
                else:
                    fig.data[dat_idx].showlegend = True  # only show once in legend
                    griffin_shown = True
        # update overall figure formatting
        fig.update_layout(height=3400, width=1150, template='simple_white', showlegend=True,
                          font={'family': 'Ubuntu', 'size': FIGURE_FONT_SIZE - 6, 'color': 'rgb(20, 20, 20)'},
                          title={'text': 'TSS GC Bias',
                                 'font': {'family': 'Ubuntu', 'size': FIGURE_FONT_SIZE + 8, 'color': 'rgb(20, 20, 20)'},
                                 'xanchor': 'center', 'yanchor': 'middle', 'x': 0.48},
                          legend=dict(orientation='h', xanchor='center', yanchor='top', x=0.5, y=-0.025,
                                      traceorder='reversed', font={'size': FIGURE_FONT_SIZE - 5}))
        # format subplot titles
        fig.update_annotations(font={'family': 'Ubuntu', 'size': FIGURE_FONT_SIZE, 'color': 'rgb(20, 20, 20)'})
        fig.update_yaxes(showgrid=True, gridwidth=2, row=1, col=1)
        for row in range(1, 8):  # turn on y-grid
            fig.update_xaxes(showgrid=True, dtick=250, gridwidth=2, row=row, col=1, range=[-1000, 1000])
            if row == 1:
        ...
        # set same y-range for PAU and for HK gene DoC Griffin_matrix_plots (separately)
        ...
        if show_figure:
            fig.show()
        output_file_path = Path(output_file_str.format(preset_str))
        output_file_path.parent.mkdir(parents=True, exist_ok=True)
        fig.write_image(output_file_path)
    # successfully created 2 figures finished
    return 0
