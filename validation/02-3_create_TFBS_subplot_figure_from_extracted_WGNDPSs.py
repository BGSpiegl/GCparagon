...
tfbs_coverage_csv_paths = {'GCparagon': {'preset1': CONTENT_ROOT_DIR /
                                                   'validation/02_loci_overlay_central_coverages/GCparagon/preset1',
                                         'preset2': CONTENT_ROOT_DIR /
                                                   'validation/02_loci_overlay_central_coverages/GCparagon/preset2'},
...
output_file_str = str(CONTENT_ROOT_DIR /
                      ...
                      'DoC_bias_correction_effect_TFBSs_{}.png')  # MUST be formatted with preset
...
def load_tfbs_regions_from_bed_file(region_list_dict: Dict[str, Union[str, Path]]) -> Dict[str, Tuple[str, int, int]]:
    tfbs_data = {}.fromkeys(region_list_dict)
    for list_name, list_path in region_list_dict.items():
        with open(list_path, mode='rt') as f_gene_list:
            tfbs_data.update({list_name: tuple(map(lambda t: (t[0], int(t[1]), int(t[2])),
                                                   filter(lambda e: e != [''],
...
def get_multi_signals_plot(plot_data_signals: Dict[str, np.array], data_category: str, x_axis_data_label: str,
                           y_axis_data_label: str,  preset: str, signal_colors=None, annotation=None) \
        -> Union[go.Figure, int]:
    """
    ...
    for signal_id in signal_id_ordered:  # original, corrected
        if signal_id == 'corrected':
            signal = plot_data_signals[signal_id][preset]  # np.array
        else:
            signal = plot_data_signals[signal_id]  # np.array
    ...
    for dat_idx in range(len(loci_group_gc_fig.data)):
        ...
        match correction_status:
            ...
            case 'corrected':
                loci_group_gc_fig.data[dat_idx].line.width = 4.5
            case 'corrected (Griffin)':
                loci_group_gc_fig.data[dat_idx].line.width = 3.
...
def main() -> int:
    ...
    # ------------------------------------------------------------------------------------------------------------------
    # read target genes
    hg38_ref_regions = load_tfbs_regions_from_bed_file(region_list_dict=target_regions_per_class)
    ...
    # ------------------------------------------------------------------------------------------------------------------
    cov_data = {}
    for corr_algo, npz_parent_dir_dict in tfbs_coverage_csv_paths.items():
        if corr_algo == 'GCparagon':
            presets = tfbs_coverage_csv_paths[corr_algo].keys()
            for preset in presets:
                npz_parent_dir = npz_parent_dir_dict[preset]
                ...
                if len(npz_candidates) == 0:
                    ...
                    continue
                for npz_candidate in npz_candidates:
                    gene = npz_candidate.stem.split('_')[1]
                    if gene not in ('LYL1', 'GRHL2'):
                        continue  # these are HK ad PAU signals. We do not plot them here!
                    ...
                    if cov_data.get(sample_name) is None:
                        cov_data.update({sample_name: {gene: {signal_type: {}.fromkeys(presets)}}})
                    elif cov_data[sample_name].get(gene) is None:
                        cov_data[sample_name].update({gene: {signal_type: {}.fromkeys(presets)}})
                    elif cov_data[sample_name][gene].get(signal_type) is None:
                        cov_data[sample_name][gene].update({signal_type: {}.fromkeys(presets)})
                    elif cov_data[sample_name][gene][signal_type].get(preset) is not None:
                        raise AssertionError(f"INSANITY - loaded coverage data already exists for algorithm '{corr_algo}', "
                                             f"sample '{sample_name}' and TF gene '{gene}' "
                                             f"and {preset} - should not be the case!")
                    cov_data[sample_name][gene][signal_type][preset] = c61_filtered_average_covs  # add avg. signal
        else:
            ...
            for npz_candidate in npz_candidates:
                ...
                if corr_algo == 'Original':
                    ...
                    assert corr_algo == 'Griffin'
                    signal_type = 'corrected (Griffin)'
                c80_filtered_average_covs = np.load(npz_candidate)['trimmed_mean_of_normalized_signals']
                if cov_data.get(sample_name) is None:
                ...
                cov_data[sample_name][gene][signal_type] = c80_filtered_average_covs  # add avg. signal

    # ACTUALLY CREATE PLOTS:
    # ------------------------------------------------------------------------------------------------------------------
    for preset_str in tfbs_coverage_csv_paths['GCparagon'].keys():
        # create Multiplot:
        ...
        h01_lyl1_traces = get_multi_signals_plot(plot_data_signals=cov_data['H01']['LYL1'], data_category='LYL1',
                                                 signal_colors=SAMPLE_COLORS, y_axis_data_label='relative coverage / 1',
                                                 x_axis_data_label='relative position / bp', annotation='H01',
                                                 preset=preset_str)
        ...
        p01_lyl1_traces = get_multi_signals_plot(plot_data_signals=cov_data['P01']['LYL1'], data_category='LYL1',
                                                 signal_colors=SAMPLE_COLORS, y_axis_data_label='relative coverage / 1',
                                                 x_axis_data_label='relative position / bp', annotation='P01',
                                                  preset=preset_str)
        ...
        b01_grhl2_traces = get_multi_signals_plot(plot_data_signals=cov_data['B01']['GRHL2'], data_category='GRHL2',
                                                  signal_colors=SAMPLE_COLORS, annotation='B01', preset=preset_str,
        ...
        h01_grhl2_traces = get_multi_signals_plot(plot_data_signals=cov_data['H01']['GRHL2'], data_category='GRHL2',
                                                  signal_colors=SAMPLE_COLORS, annotation='H01', preset=preset_str,
                                                  x_axis_data_label='relative position / bp',
                                                  y_axis_data_label='relative coverage / 1')
        ...
        p01_grhl2_traces = get_multi_signals_plot(plot_data_signals=cov_data['P01']['GRHL2'], data_category='GRHL2',
                                                  signal_colors=SAMPLE_COLORS, annotation='P01', preset=preset_str,
                                                  x_axis_data_label='relative position / bp',
                                                  y_axis_data_label='relative coverage / 1')
        ...
        fig.update_annotations(font={'family': 'Ubuntu', 'size': 32, 'color': 'rgb(20, 20, 20)'})
