...
#
# WARNING - THIS CODE IS COMPUTATIONALLY EXPENSIVE!
...
hg38_2bit_ref_genome = SOURCE_CODE_ROOT_PATH.parent.parent.parent / 'src/GCparagon/2bit_reference/hg38.analysisSet.2bit'
# needs to be downloaded using the EXECUTE_reference_download.sh script
hg38_ref_genome_chrom_sizes = SOURCE_CODE_ROOT_PATH.parent.parent.parent / \
...
GRIFFIN_MATRICES_PATH = SOURCE_CODE_ROOT_PATH.parent.parent.parent / "validation/transformed_Griffin_bias_matrices"

# ---------------------- SELECT CORRECTION ALGORITHM ----------------------
# CORRECTION_ALGORITHM = 'Griffin'  # must be from ('Griffin', 'GCparagon')
# -------------------------------------------------------------------------

out_path = SCRIPT_ROOT_PATH / f'simulated_hg38_ref_gc_fromEntireBAM'
# observed_fragments = observed_fragment_dicts[CORRECTION_ALGORITHM]
observed_fragments_tsv_file = SOURCE_CODE_ROOT_PATH.parent.parent.parent / \
                              ("validation/genomewide_GC_content_per_fragment_corrected/"
                               "original_and_corrected_gc_distribution-COMBINED/"
                               "STATISTICS_allSamplesAlgorithms_GC-percentage_perFragmentSequence_14-08-2023.tsv")
# ^------ FILE ABOVE CREATED WITH 02_create_corrected_fragment_gc_distributions.py
...
def main():
    ...
    for sample_id, flength_dist in fragments_per_sample.items():
        # read sizes of standard ref scaffolds and compute sampling proportions
